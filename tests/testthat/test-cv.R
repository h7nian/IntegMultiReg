...
test_that("the cross-validated metric reflects the outcome type", {
  ...
  expect_identical(cv_imr(fc, k = 5, rounds = 1)$metric, "MSE")
  fs <- fit_demo("right.censored", total = 200, burn = 100, seed = 4)
  expect_identical(cv_imr(fs, k = 5, rounds = 1)$metric, "C-index")
})
...
test_that("cv_imr validates cross-validation controls", {
  ...
  expect_error(cv_imr(fit_bin, k = max(fit_bin$model$sample_sizes) + 1), "sample size")
