...
`cv_imr()` defaults to `cv_method = "legacy"`, the historical post-fit
algorithm. `cv_method = "importance"` uses all retained draws in a paper-derived
importance average; both post-fit modes reuse full-fit preprocessing and latent
response summaries. Neither promises reproduction of the original study's numbers.

This example explicitly uses `cv_method = "refit"` to refit each training fold, including
standardization, selection MCMC and model weights. It reports AUC for binary
outcomes, concordance for survival and MSE for continuous outcomes, overall
and within each retained availability subgroup. Formula transformations are
also rebuilt on training rows. Runtime scales with folds times rounds; use
short chains for a code-path check and increase them for final analysis.
To compare IMR with BMS, fit a separate `method = "bms"` object. The original
hyperparameters are treated as fixed; tuning requires another validation layer.
...
`outcome_type` and the `outcome` data frame change.
...
## Choosing computational conventions

The three CV modes provide defaults. Optional `ridge`, `model_set`,
`df_method`, and `score_method` let users choose unpenalized estimation,
empirical or ranked selection states, predictive degrees of freedom, and
scoring independently. `folds = result$control$folds` replays actual partitions
and summation order; identical seeds across different generators are not enough.
`fold_rng="continue"` resumes a new fit's saved GSL stream for post-fit CV.

`sampler_method="paper"` uses the symmetric MRF conditional and corrects
boundary proposal probabilities. The default `"legacy"` preserves historical
updates, whose target differs from the stated MRF posterior. Numerical options
`prior_indexing`, `laplace_max_iter`, and `laplace_tolerance` are recorded and
reused by prediction and CV. See `system.file("METHOD-COVERAGE.md",
package="IntegMultiReg")` for reference combinations and their validation scope.
No mode name alone guarantees reproduction of historical experiment tables.
